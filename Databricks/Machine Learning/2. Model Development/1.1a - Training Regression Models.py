...
feature_data_pd = fe.read_table(name=table_name).toPandas() # Resulting Pandas pandas.DataFrame is expected to be small, as all the data is loaded into the driver’s memory
...
for i in range(len(corr.columns)):
    for j in range(len(corr.columns)):
        ...
        plt.gca().add_patch(plt.Circle((j, i), 
                                       0.5 * np.abs(corr.iloc[i, j]), # to reduce the area of the circle to within distinct boxes
                                       color=color, 
                                       edgecolor=color,
                                       fill=fill,
                                       alpha=0.5))
...
plt.xlim(-0.5, len(corr.columns) - 0.5) # adjust circles to the center (left-to-right)
plt.ylim(-0.5, len(corr.columns) - 0.5) # adjust circles to the center (top-to-bottom)
...
std_ct = ColumnTransformer(transformers=[("scaler",
                                          StandardScaler(),
                                          ["total_bedrooms", "total_rooms", "housing_median_age", "latitude", "longitude", "median_income", "population", "ocean_proximity", "households"])])
...
# NOTE: The RMSE provides a the variance in price from the predictions to the actual values using the same units. Thus, our predictions are typically $72,400 different the actual value.
