...
test_features_df = test_df.select(primary_key, *features) # NOTE: *features will ensure response is not unpacked, i.e., churn
...
# Reading [Entity Considerations - Feature Aggregation](https://community.databricks.com/t5/technical-blog/mlops-gym-databricks-feature-store/ba-p/67430) for a good review of aggregate considerations when your data is relevant over a specified period of time (e.g., numerous customer interactions with customer interactions within X days/months = churn prediction). First of three articles to review.
...
# Note that this feature table includes all features from the table, training and test set features less labels... primary key will be excluded from the training set
fl_handle = FeatureLookup(
    table_name=feature_table_name, # Feature store 3-level namespace
...
training_set_spec = fe.create_training_set(
    ...
    label=response,
    feature_lookups=[fl_handle], # Can include multiple if pulling features from several tables
