...
partial class Program
{
    static void FilterAndSort()
    {
        ...
        using (Northwind db = new())
        {
            ...
            IQueryable<Product> sortedAndFilteredProducts =
    ...
    {
        SectionTitle("Join categories and prodcuts");
    ...
    static void AggregateProducts()
    {
        ...
        using (Northwind db = new())
        {
            ...
            {
                WriteLine("Products does not have a Count property.");
            }
    ...
    static void OutputTableOfProducts(Product[] products, int currentPage, int totalPages)
    {
        ...
        WriteLine(line);
        WriteLine("{0,4} {1,-40} {2,12:C} {3,-15}", "ID", "Product Name", "Unit Price", "Discontinued");
        WriteLine();
    ...
    static void OutputPageOfProductsSkipAndTake(IQueryable<Product> products, int pageSize, int currentPage, int totalPages)
    {
    ...
    static void OutputPageOfProductsChunk(IQueryable<Product> products, int pageSize, int currentPage, int totalPages)
    {
        // We must order data before skipping and taking to ensure
        // the data is not randomly sorted in each page
        Product[] pageOfProducts = products.OrderBy(p => p.ProductId)
            .AsEnumerable()
            .Chunk(pageSize)
            .ElementAt(currentPage);

        OutputTableOfProducts(pageOfProducts, currentPage, totalPages);
    }
    ...
    static void PagingProducts()
    {
        ...
        using (Northwind db = new())
        {
            ...
            while (true)
            {
                OutputPageOfProductsSkipAndTake(db.Products, pageSize, currentPage, totalPages);
                OutputPageOfProductsChunk(db.Products, pageSize, currentPage, totalPages);

                WriteLine("Press <- to page back, press -> to page forward, any key to exit.");
    ...
    static void CustomExtensionMethods()
    {
        ...
        using (Northwind db = new())
        {
            ...
            WriteLine("{0,-25} {1,10:$#,##0.00}", "Median unit price:", db.Products.Median(p => p.UnitPrice));
            WriteLine("{0,-25} {1,10:N0}", "Mode units in stock:", db.Products.Mode(p => p.UnitsInStock));
            WriteLine("{0,-25} {1,10:$#,##0.00}", "Mode unit price:", db.Products.Mode(p => p.UnitPrice));
        }
    }
